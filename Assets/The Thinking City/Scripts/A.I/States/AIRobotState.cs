...
public abstract class AIRobotState : AIState 
{
	...
	// -------------------------------------------------------------------------------------
	// Name	:	OnTriggerEvent
	// Desc	:	Called by the parent state machine when threats enter/stay/exit the Robot's
		...
		if (eventType != AITriggerEventType.Exit) 
		{
			...
			if (other.CompareTag ("Player")) {
				...
				if (curType != AITargetType.Visual_Player ||
				    (curType == AITargetType.Visual_Player && distance < _robotStateMachine.VisualThreat.distance)) {
			...
			if (other.CompareTag ("AI Food") &&	curType!=AITargetType.Visual_Player &&	curType!=AITargetType.Visual_Light &&
				_robotStateMachine.satisfaction<=0.9f && _robotStateMachine.AudioThreat.type==AITargetType.None   )
			{	
	...
	// Desc	:	Test the passed collider against the Robot's FOV and using the passed
	...
	protected virtual bool	ColliderIsVisible( Collider other,  out RaycastHit hitInfo, int layerMask=-1 )
	{
		...
		// of our sensor radius scaled by the Robot's sight ability. This will return ALL hits.
