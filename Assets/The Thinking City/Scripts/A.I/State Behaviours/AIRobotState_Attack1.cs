...
// Desc		: A Robot state used for attacking the player when in range
// -----------------------------------------------------------------
public class AIRobotState_Attack1 : AIRobotState
{
    ...
    public override AIStateType OnUpdate()
    {
        ...
        if (_robotStateMachine.VisualThreat.type == AITargetType.Visual_Player)
        {
            ...
            // If we are not in melee range any more than back to pursuit state
            if (!_robotStateMachine.inMeleeRange) return AIStateType.Pursuit;

            if (!_robotStateMachine.useRootRotation)
            {
                // Keep the robot facing the player at all times
        ...
        // Player has stepped outside out FOV or hidden so face pplayers direction and then
        // drop back to Alerted state to give the A.I a chance to re-aquire the target
        if (!_robotStateMachine.useRootRotation)
        {
            targetPos = _robotStateMachine.targetPosition;
            targetPos.y = _robotStateMachine.transform.position.y;
