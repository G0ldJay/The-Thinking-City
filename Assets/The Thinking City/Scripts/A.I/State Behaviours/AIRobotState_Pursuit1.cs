...
public class AIRobotState_Pursuit1 : AIRobotState
{
    ...
    public override AIStateType OnUpdate()
    {
        ...
        if (_stateMachine.targetType == AITargetType.Visual_Player && _robotStateMachine.inMeleeRange)
        {
        ...
        if (_robotStateMachine.navAgent.isPathStale ||
            !_robotStateMachine.navAgent.hasPath ||
            _robotStateMachine.navAgent.pathStatus != UnityEngine.AI.NavMeshPathStatus.PathComplete)
        {
            GameManager.instance.PlayAISound(5);
        ...
        if (!_stateMachine.useRootRotation && !_robotStateMachine.isTargetReached)
        {
